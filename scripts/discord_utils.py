...
def build_todo_content(fields: dict, venue_url) -> str:
    ...
    (サーバーメンバーなら誰でも閲覧・参加可能)に投稿される。会場URLは申込み時の
    必須項目のため、承認時点では必ず存在する前提。ただし「申込み必須」イベントでは
    会場URLを公開スレッドに一切出さず、参加申込み者への個別メール案内に切り替える
    (告知メッセージ側も同様)。「申込み必須」でない通常イベントの会場URLは
    告知メッセージ(スレッド最初の投稿)の冒頭に既に記載されているため、ここでは
    重ねて表示しない。

    NotionページのURLは、Discordの本文中に丸括弧などで直接埋め込むと自動リンク化
    されないため、必ず行頭(直前が空白または改行)に単独で置く。
    """
    material_folder_url = (
    ...
    notion_url = notion_page_url(fields["page_id"])

    if fields.get("requires_rsvp"):
        venue_section = (
            f"**会場URL**: 事前申込み制のため、参加申込みフォームからお申し込みいただいた方に"
            f"個別にメールでご案内します(このスレッドには掲載されません)。\n"
            f"　※主催者であるあなたも参加者の一人として自動登録済みです。フォームへの"
            f"記入は不要で、他の参加者と同様に会場URLの案内メール・開催直前のリマインダー"
            f"メールが届きます。\n\n"
        )
    else:
        # 通常イベントの会場URLは告知メッセージ冒頭に既出のため、ここでは表示しない。
        venue_section = ""
    ...
    return (
        ...
        f"　　アップロードした資料のURLは、下記の「本イベント用のNotionページ」の"
        f"「資料リンク」欄に記入してください。\n"
        f"　　{notion_url}\n\n"
        f"□ 2. 資料のアップロードは任意ですが、2日前までに資料URLの共有が確認できない場合、"
        f"このスレッドにリマインダーが自動投稿されます。\n\n"
        f"□ 3. 前日と、開催30分前に「#🐸｜井戸端かいぎ」チャンネル全体へ、このスレッドへの"
        f"リンクつきでリマインダーが届きます。\n\n"
        f"**登録内容を修正したいとき**\n"
        f"・下記の「本イベント用のNotionページ」から、登録内容(日時・概要など)を修正できます。\n"
        f"　{notion_url}\n"
        f"・修正後、そのページの「更新情報をスレッドに通知する」ボタンを押すと、更新情報を通知できます。\n"
        f"　・このスレッドの最初の投稿(変更箇所に取り消し線)が更新されます。\n"
        f"　・日時を変更した場合は、加えて「#🐸｜井戸端かいぎ」チャンネル全体へも通知が届きます。\n"
        f"　・ボタンを押すまでは通知されないため、修正が全て終わってから押してください"
        f"(更新と通知は何度でも可能です)。\n"
        f"　・編集にはNotionのアカウント作成が必要です。アカウントを作成しない場合は、"
        f"綿引({admin_mention})に編集内容をお伝えいただければ代わりに反映します。\n"
        f"　・ボタンを押してから実際に通知が届くまで、60〜120分以上かかる場合があります"
        f"(時間に余裕をもって操作してください)。\n\n"
        f"**イベントが複数回にわたる場合**\n"
        f"・下記の「本イベント用のNotionページ」を開き、複製ボタンを押すとページが複製されます。\n"
        f"　{notion_url}\n"
        f"・複製されたページには日時が記入されていないので、次回の開催日時を記入し、"
        f"「ステータス」を「確定」にしてください。\n"
        f"・この案内と同じ流れで新しいスレッドが自動的に作成されます"
        f"(会場URLも前回と同じものが自動的に引き継がれます)。\n\n"
        f"**やむを得ずキャンセルする場合**\n"
        f"・そのイベントのNotionページで「ステータス」を「キャンセル」に変更してください。\n"
        f"・カレンダーの予定が自動的に削除され、このスレッドと「#🐸｜井戸端かいぎ」"
        f"チャンネル全体にキャンセルの通知が届きます。\n\n"
        f"主催者の操作に関してご不明な点があれば、綿引({admin_mention})までお気軽にどうぞ。"
    )
