...
def build_todo_content(fields: dict, venue_url) -> str:
    ...
    venue_urlが設定済みで「申込み必須」でもない場合は、告知メッセージの冒頭に
    既に会場URLが記載されているため、ここでは重ねて表示しない。
    ...
    return (
        ...
        f"　　アップロードした資料のURLは「本イベント用のNotionページ」({notion_url})の"
        f"「資料リンク」欄に記入してください。\n\n"
        f"□ 2. 資料のアップロードは任意ですが、2日前までに資料URLの共有が確認できない場合、"
        f"このスレッドにリマインダーが自動投稿されます。\n\n"
        f"□ 3. 前日と、開催30分前に「#🐸｜井戸端かいぎ」チャンネル全体へ、このスレッドへの"
        f"リンクつきでリマインダーが届きます。\n\n"
        f"□ 4. 主催者は、ご自身のイベントページ({notion_url})の内容を修正したら、"
        f"そのページの「更新情報をスレッドに通知する」ボタンを押すことで更新情報を通知できます。\n"
        ...
        f"　・ボタンを押してから実際にスレッドへ通知が届くまで、最大10分程度のタイムラグがあります。\n\n"
        f"**イベントが複数回にわたる場合**\n"
        f"・このイベントのNotionページを複製し、日時だけを変更して「ステータス」を「確定」にしてください。\n"
        f"・そのつどこの案内と同じ流れで新しいスレッドが自動的に作成されます。\n"
