...
1. `.github/workflows/poll.yml` が10分おきに自動実行され、
   - `poll_approve.py`: 「ステータス」が「確定」なのに「Discordスレッド」が
     未作成の行を検知し、
     - 「複数回」の場合、リレーション「シリーズ」から「井戸端かいぎ シリーズ」
       データベースの正データ(タイトル・種別・概要・対象・発表者氏名)を取得
     - Zoomミーティングを作成(単発は毎回新規。複数回は同じシリーズページに
       紐づく既存のZoomリンクを再利用し、なければ新規に
       「時間固定なし」ミーティングを作成)
...
#### 5-1. 「井戸端かいぎ シリーズ」データベースを新規作成

「複数回」開催(輪読会など)の正データを1箇所にまとめるための、独立したデータベースです。
以下のプロパティを作成してください:
...
作成したら、このデータベースにも既存のNotion integration(`NOTION_TOKEN`発行元)を
「接続」として追加するのを忘れないでください。予定表側と同じ手順です。

#### 5-2. 「井戸端かいぎの予定表」データベースの設定

以下のプロパティが必要です(名称は完全一致させてください):
タイトル / 日時 / 種別 / 発表者氏名(テキスト) / 主催者ユーザ名(テキスト) /
ステータス / 概要 / 対象 / 資料リンク / 録画 / 議事メモ /
Discordスレッド(URL型) / 開催頻度(セレクト: 単発・複数回) /
**シリーズ(リレーション型。5-1で作った「井戸端かいぎ シリーズ」データベースを参照)**

「シリーズ」をリレーションにしたら、シリーズ側のタイトル・種別・概要・対象・
発表者氏名を**ロールアップ**として予定表に追加すると、シリーズを選んだ瞬間に
それらの内容が画面上に表示されるようになります(例: 「シリーズ・タイトル」
「シリーズ・種別」…といったロールアッププロパティを追加)。
これは表示用で、実際の自動化処理(`poll_approve.py`)はリレーション先の
シリーズページを直接読みに行くため、ロールアップの有無に関わらず動作します。
...
「開催頻度」で複数回、「シリーズ」で既存のシリーズを選択(または新規作成)、
「日時」にその回の日時だけを入力してもらえば十分です。承認時に、シリーズ側の
タイトル・種別・概要・対象・発表者氏名が自動的に使われます(予定表の行自体には
書き戻しません。ロールアップで表示されるため)。

新しいシリーズをフォームから追加できるかどうかは、Notionのフォーム機能の
バージョン・プランに依存します。実際にフォームを開き、「シリーズ」欄で
新規項目をその場で作成できるか確認してください。対応していない場合は、
「井戸端かいぎ シリーズ」データベース専用の小さな別フォームを用意し、
まずシリーズを登録してから予定表フォームでそれを選んでもらう、という
二段階の運用に切り替えてください。

「ステータス」を「キャンセル」に変更する、またはページ自体を削除(ゴミ箱に移動)
すると、対応するGoogleカレンダーの予定が自動的に削除されます
(`sync_cancellations.py`)。日時を変更したい場合の直接編集には対応していません。
いったんキャンセルし、新しい日時で登録し直す運用です。
...
- `remind_events.py` のリマインダー、複数回シリーズのZoomリンク再利用、
  ...
  (NotionページID・DiscordスレッドID/URL・ZoomリンクURL・シリーズページID)を参照します。
