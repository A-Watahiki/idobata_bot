...
**対外公開するビューでは必ずこの列を非表示にしてください。**
自動化コードはこのプロパティを一切読み書きしません。

注意: Discordスレッドに送る「本イベント用のNotionページ」リンクは、その回の
イベントの行ページそのものを指しています。行ページを直接開くと、テーブル
ビューでの列の絞り込みとは関係なく、既定では**その行が持つ全プロパティが
表示されます**。「メールアドレス」など見せたくない項目は、データベースの
「ページプロパティ」設定(各行を開いたときに、どのプロパティを表示するか)
からも非表示にしてください。これはビュー単位ではなくデータベース全体の設定です。

またNotionの共有・Web公開設定は**ページ単位**であり、ビュー単位ではありません。
「テーブルビュー(運営用)」「フォーム(運営用)」「カレンダー(公開)」
「予定表(公開)」のように1つのページに複数ビューをまとめている場合、
ページ全体を「Webに公開」すると、URLの`?v=`部分を書き換えるだけで
運営用ビューにも到達できてしまいます。運営用ビューを本当に保護したい場合は、
公開用ビューだけを別ページに分離するか、データベース自体を分けてください。
